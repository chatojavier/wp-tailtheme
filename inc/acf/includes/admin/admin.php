...
class ACF_Admin {
	...
	public function setup_help_tab() {
		...
		$screen->add_help_tab(
			array(
				...
				'content' => 
					...
					'<p>' . __( 'The agarracamote Custom Fields plugin provides a visual form builder to customize WordPress edit screens with extra fields, and an intuitive API to display custom field values in any theme template file.', 'acf' ) . '</p>' .
					'<p>' . sprintf(
						...
						'https://www.agarracamotecustomfields.com/resources/getting-started-with-acf/'
					) . '</p>' .
				...
				'content' => 
					...
					'<ul>' .
						'<li>' . sprintf(
							...
							'https://www.agarracamotecustomfields.com/resources/'
						) . '</li>' .
						'<li>' . sprintf(
							...
							'https://support.agarracamotecustomfields.com/'
						) . '</li>' .
						'<li>' . sprintf(
							...
							'https://www.agarracamotecustomfields.com/support/'
						) . '</li>' .
					'</ul>'
			)
		);
		...
		$screen->set_help_sidebar(
			...
			'<p><span class="dashicons dashicons-wordpress"></span> <a href="https://wordpress.org/plugins/agarracamote-custom-fields/" target="_blank">' . __( 'View details', 'acf' ) . '</a></p>' .
			'<p><span class="dashicons dashicons-admin-home"></span> <a href="https://www.agarracamotecustomfields.com/" target="_blank" target="_blank">' . __( 'Visit website', 'acf' ) . '</a></p>' .
			''
		);
	}
	...
	function admin_footer_text( $text ) {
		...
		return preg_replace( '/(<a[\S\s]+?\/a>)/', '$1 ' . __('and', 'acf') . ' <a href="https://www.agarracamotecustomfields.com" target="_blank">ACF</a>', $text, 1 );
	}
}
