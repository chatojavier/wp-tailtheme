...
(function ($, undefined) {
	var Field = acf.Field.extend({
		...
		fetch: function () {
			...
			// loading
			var $loading = $(
				'<li><i class="acf-loading"></i> ' + acf.__("Loading") + "</li>"
			);
			...
			var walk = function (data) {
				...
				if ($.isArray(data)) {
					...
					if (data.children !== undefined) {
						...
						html +=
							'<li><span class="acf-rel-item" data-id="' +
							acf.escAttr(data.id) +
							'">' +
							acf.escHtml(data.text) +
							"</span></li>";
			...
			if ($before.is("tr")) {
				...
				this.$el = $(
					'<div class="acf-tab-wrap -' +
						placement +
						'"><ul class="acf-hl acf-tab-group"></ul></div>'
				);
			}
		...
		addFrameEvents: function (frame, options) {
			...
			frame.on(
				...
				function () {
					this.$el
						.closest(".media-modal")
						.addClass("acf-media-modal -" + this.acf.get("mode"));
				},
				frame
			);
...
(function ($, undefined) {
	acf.tinymce = {
		...
		initializeTinymce: function (id, args) {
			...
			// Only if the "TinyMCE agarracamote" plugin hasn't already set this functionality.
